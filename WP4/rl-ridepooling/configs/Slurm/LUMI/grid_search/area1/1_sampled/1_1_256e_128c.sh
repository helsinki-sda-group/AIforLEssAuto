...
srun exec $SIF python -c "import torch; print('mkldnn:', torch.backends.mkldnn.enabled)"

srun --hint=compute_bound singularity exec --bind /usr/lib64 $SIF python src/tests/gym_test-rs.py --config configs/policy_training/helsinki_updated_areas/area1_sampled_1.yaml --total-iters 256 --num-envs 256 --postfix ${SLURM_JOB_ID}_${SLURM_JOB_NAME}
