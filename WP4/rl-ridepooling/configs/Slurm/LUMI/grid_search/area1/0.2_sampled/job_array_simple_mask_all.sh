#!/bin/bash
#SBATCH --array=0-14
#SBATCH --job-name="ja_area1-3_all_percentages"
...
export AREAS_STR="area1:area2:area3"
export PERCENTAGE_SAMPLED_STR="0.2:0.4:0.6:0.8:1"
...
for area in "${AREAS[@]}"; do
    for percent in "${PERCENTAGES[@]}"; do
        COMBINATIONS[$index]="$area:$percent"
        ((index++))
    done
done
...
IFS=":" read -r AREA PERCENTAGE_SAMPLED <<< "$combo"

echo "Selected AREA: $AREA"
echo "Selected PERCENTAGE_SAMPLED: $PERCENTAGE_SAMPLED"

output_file_base="output/${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}_${SLURM_JOB_NAME}_${AREA}_${PERCENTAGE_SAMPLED}"

srun --ntasks=7 --kill-on-bad-exit=0 --output="${output_file_base}/%t.log" --error="${output_file_base}/%t.err" ${cpu_bind} bash -c '
    ...
    taskset -cp $$
    hybrid_check -n -r
    singularity exec --bind /usr/lib64 $SIF python src/tests/gym_test-rs.py --config configs/policy_training/helsinki_updated_areas/${AREA}_sampled_${PERCENTAGE_SAMPLED}.yaml --total-iters $NUM --num-envs $NUM --prefix ${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}_${SLURM_PROCID}_${NUM}e_${SLURM_JOB_NAME}
